...
### Routing & Views
- **`/` — Public RSVP Form:** Public landing page. Prompts guests for their `name` and `email` using client-side and server-side validation, writes the entry to MongoDB, triggers a Resend confirmation email, and redirects to success.
...
- **Clean DB Connections:** Leverage the cached connection pattern in `src/lib/mongodb.ts` to avoid multiple active connections during hot-reloads and serverless executions.
...
### Ask First
- **Schema Updates:** Ask before modifying the Mongoose model schema or adding new fields to the RSVP document.
...
- **Direct DB Queries in Components:** Never perform MongoDB query operations directly inside React Components; route all requests through server actions in `src/lib/actions.ts`.
...
│   │   ├── mongodb.ts     # MongoDB caching connection singleton
│   │   └── validation.ts  # Regex syntax & domain MX record checking
│   └── models/            # Mongoose Schemas & TypeScript interfaces
│       └── rsvp.ts        # RSVP model schema definition
...
export async function exampleAction(data: { email: string }) {
  ...
  try {
    ...
    await Rsvp.create({ email: data.email });
    return { success: true };
  } catch (err) {
...
- Always cast Mongoose documents to their appropriate Typescript schemas.
...
When testing server actions, ensure that `mongodb`, Mongoose models, and `next/headers` cookie stores are fully mocked:
